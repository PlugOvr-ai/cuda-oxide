...
///
/// - `acc_ptr` (ptr): pointer to the 4-f32 accumulator (read-modify-write)
/// - `a0..a3` (u32): A fragment, 4 tf32-in-b32 registers
/// - `b0,b1` (u32): B fragment, 2 tf32-in-b32 registers
///
/// # Results
///
/// - None (accumulator is updated in-place via pointer)
...
impl Verify for MmaSyncM16N8K8F32Tf32Op {
    fn verify(&self, ctx: &Context) -> Result<(), Error> {
        let op = self.get_operation().deref(ctx);
        if op.get_num_operands() != 7 || op.get_num_results() != 0 {
            return verify_err!(
                op.loc(),
                "nvvm.mma_sync_m16n8k8_f32_tf32 requires seven operands and no results"
            );
        }
        let accumulator_ty = op.get_operand(0).get_type(ctx);
        if accumulator_ty
            ...
            .is_none()
        {
            return verify_err!(
                op.loc(),
                "nvvm.mma_sync_m16n8k8_f32_tf32 accumulator must be a MIR pointer"
            );
        }
        // Operands 1..=6 are the A (a0..a3) and B (b0,b1) fragment registers,
        // each a b32 lane value carrying a packed tf32.
        for idx in 1..7 {
            if !is_u32(ctx, op.get_operand(idx).get_type(ctx)) {
                return verify_err!(
                    op.loc(),
                    "nvvm.mma_sync_m16n8k8_f32_tf32 fragment registers must be u32"
                );
            }
        }
        Ok(())
