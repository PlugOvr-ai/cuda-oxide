...
impl OnnxExecutor {
    ...
    fn op_conv(&self, node: &NodeProto, tensors: &mut TensorMap) -> Result<()> {
        ...
        let mut result_buf =
            unsafe { DeviceBuffer::<f32>::uninitialized_async(&self.stream, out_numel) }
    ...
    fn op_matmul(&self, node: &NodeProto, tensors: &mut TensorMap) -> Result<()> {
        ...
        let mut out =
            unsafe { DeviceBuffer::<f32>::uninitialized_async(&self.stream, batch * m * n) }
    ...
    fn op_split(&self, node: &NodeProto, tensors: &mut TensorMap) -> Result<()> {
        ...
        for (oi, &sz) in sizes.iter().enumerate() {
            ...
            let mut piece =
                unsafe { DeviceBuffer::<f32>::uninitialized_async(&self.stream, piece_numel) }
