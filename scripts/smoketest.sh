...
ERROR_EXAMPLES=(error error_wgmma_mma_unimplemented error_set_discriminant_uninhabited error_enum_constant_provenance error_enum_pointer_overlap error_enum_shared_pointer_layout error_static_initializer_provenance error_static_slice_addend error_static_slice_unsize error_tuple_array_provenance error_tuple_constant_provenance error_struct_constant_provenance error_heap_alloc error_missing_device_attr error_generated_intrinsic_abi error_generated_intrinsic_unknown_id error_generated_intrinsic_fn_pointer error_generated_intrinsic_callable)
...
verdict_error() {
    ...
    case "${ex}" in
        ...
        error_static_slice_unsize)
            if ! grep -Fq 'zero-addend pointee reinterpretations and unsized coercions are not supported' "${log}"; then
                echo "FAIL (missing zero-addend static slice-coercion diagnostic)"
                return 1
            fi
            ;;
